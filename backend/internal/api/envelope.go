...
// ErrorWithCode preserves the numeric HTTP/envelope code while exposing a
// stable machine-readable reason for client recovery flows. Human-readable
// messages remain unsuitable as an API contract and may be localized.
func ErrorWithCode(c *gin.Context, statusCode int, errorCode, msg string) {
	c.JSON(statusCode, envelope{
		Code:      statusCode,
		Data:      nil,
		Message:   msg,
