...
p=sys.argv[1]
pat=re.compile(r"[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{20,}")
s=open(p,encoding="utf-8",errors="replace").read()
n=pat.sub("<REDACTED:credential>",s)
if n!=s:
    open(p,"w",encoding="utf-8").write(n)' "$f" 2>/dev/null || true
}
...
ST_RC="$(curl -sS -m 25 -o "$EVID/F8-reconfigure.json" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOK3" -H 'Content-Type: application/json' -d '{"baudrate":9600}' "$BASE/api/v1/channels/$CHID/reconfigure" 2>/dev/null || echo 000)"
