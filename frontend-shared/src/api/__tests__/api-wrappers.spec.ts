...
describe('userApi', () => {
  ...
  it('list calls get with params', async () => {
    mockClient.get.mockResolvedValue({ list: [], total: 0, page: 1, page_size: 20 })
    await userApi.list({ page: 1, keyword: 'admin' })
    expect(mockClient.get).toHaveBeenCalledWith('/api/v1/users', { params: { page: 1, keyword: 'admin' } })
  })

  it('list default params', async () => {
    mockClient.get.mockResolvedValue({ list: [], total: 0, page: 1, page_size: 20 })
    await userApi.list()
    expect(mockClient.get).toHaveBeenCalledWith('/api/v1/users', { params: {} })
  })

  it('get calls get with id', async () => {
    mockClient.get.mockResolvedValue({ id: 1, username: 'admin' })
    await userApi.get(1)
  ...
  it('create posts data', async () => {
    mockClient.post.mockResolvedValue({ id: 1 })
    await userApi.create({ username: 'new', password: 'pass', role: 'viewer' })
    expect(mockClient.post).toHaveBeenCalledWith('/api/v1/users', { username: 'new', password: 'pass', role: 'viewer' })
  })

  it('update puts data', async () => {
    mockClient.put.mockResolvedValue({ id: 1 })
    await userApi.update(1, { role: 'admin' })
    expect(mockClient.put).toHaveBeenCalledWith('/api/v1/users/1', { role: 'admin' })
  })

  it('delete calls delete', async () => {
    mockClient.delete.mockResolvedValue(undefined)
    await userApi.delete(1)
    expect(mockClient.delete).toHaveBeenCalledWith('/api/v1/users/1')
  })

  it('changePassword posts', async () => {
    mockClient.post.mockResolvedValue(undefined)
    await userApi.changePassword({ old_password: 'old', new_password: 'new' })
    expect(mockClient.post).toHaveBeenCalledWith('/api/v1/users/me/change-password', { old_password: 'old', new_password: 'new' })
  })

  it('resetPassword posts', async () => {
    mockClient.post.mockResolvedValue(undefined)
    await userApi.resetPassword(1, 'newpass')
    expect(mockClient.post).toHaveBeenCalledWith('/api/v1/users/1/reset-password', { new_password: 'newpass' })
  })
})

// ─── vendor ───
import { vendorApi, deviceModelApi, deviceCategoryApi } from '../vendor'

describe('vendor API', () => {
  beforeEach(() => vi.clearAllMocks())

  it('vendorApi.list calls get with params', async () => {
    mockClient.get.mockResolvedValue({ data: [], total: 0 })
    await vendorApi.list({ page: 1, page_size: 10 })
    expect(mockClient.get).toHaveBeenCalledWith('/vendors', { params: { page: 1, page_size: 10 } })
  })

  it('vendorApi.listAll passes enabled=true', async () => {
    mockClient.get.mockResolvedValue({ data: [] })
    await vendorApi.listAll()
    expect(mockClient.get).toHaveBeenCalledWith('/vendors', { params: { page_size: 100, enabled: true } })
  })

  it('vendorApi.get calls get with id', async () => {
    mockClient.get.mockResolvedValue({ data: { id: 1 } })
    await vendorApi.get(1)
    expect(mockClient.get).toHaveBeenCalledWith('/vendors/1')
  })

  it('vendorApi.create posts', async () => {
    mockClient.post.mockResolvedValue({ data: { id: 1 } })
    await vendorApi.create({ code: 'BOSCH', name: 'Bosch' })
    expect(mockClient.post).toHaveBeenCalledWith('/vendors', { code: 'BOSCH', name: 'Bosch' })
  })

  it('vendorApi.update puts', async () => {
    mockClient.put.mockResolvedValue({ data: {} })
    await vendorApi.update(1, { name: 'Updated' })
    expect(mockClient.put).toHaveBeenCalledWith('/vendors/1', { name: 'Updated' })
  })

  it('vendorApi.delete calls delete', async () => {
    mockClient.delete.mockResolvedValue(undefined)
    await vendorApi.delete(1)
    expect(mockClient.delete).toHaveBeenCalledWith('/vendors/1')
  })
})

describe('deviceModelApi', () => {
  beforeEach(() => vi.clearAllMocks())

  it('list calls get with params', async () => {
    mockClient.get.mockResolvedValue({ data: [], total: 0 })
    await deviceModelApi.list({ vendor_id: 1 })
    expect(mockClient.get).toHaveBeenCalledWith('/device-models', { params: { vendor_id: 1 } })
  })

  it('get calls get with id', async () => {
    mockClient.get.mockResolvedValue({ data: { id: 1 } })
    await deviceModelApi.get(1)
    expect(mockClient.get).toHaveBeenCalledWith('/device-models/1')
  })

  it('create posts', async () => {
    mockClient.post.mockResolvedValue({ data: { id: 1 } })
    await deviceModelApi.create({ vendor_id: 1, code: 'BMP280', name: 'BMP280', category: 'temp', protocol: 'i2c' })
    expect(mockClient.post).toHaveBeenCalledWith('/device-models', { vendor_id: 1, code: 'BMP280', name: 'BMP280', category: 'temp', protocol: 'i2c' })
  })

  it('update puts', async () => {
    mockClient.put.mockResolvedValue({ data: {} })
    await deviceModelApi.update(1, { name: 'Updated' })
    expect(mockClient.put).toHaveBeenCalledWith('/device-models/1', { name: 'Updated' })
  })

  it('delete calls delete', async () => {
    mockClient.delete.mockResolvedValue(undefined)
    await deviceModelApi.delete(1)
    expect(mockClient.delete).toHaveBeenCalledWith('/device-models/1')
  })

  it('getDefinitions calls get', async () => {
    mockClient.get.mockResolvedValue({ data: [] })
    await deviceModelApi.getDefinitions(1)
    expect(mockClient.get).toHaveBeenCalledWith('/device-models/1/definitions')
  })

  it('updateDefinitions puts', async () => {
    mockClient.put.mockResolvedValue(undefined)
    await deviceModelApi.updateDefinitions(1, [])
    expect(mockClient.put).toHaveBeenCalledWith('/device-models/1/definitions', { definitions: [] })
  })
})

describe('deviceCategoryApi', () => {
  beforeEach(() => vi.clearAllMocks())

  it('list calls get', async () => {
    mockClient.get.mockResolvedValue({ data: [] })
    await deviceCategoryApi.list()
    expect(mockClient.get).toHaveBeenCalledWith('/device-categories')
  })
})
