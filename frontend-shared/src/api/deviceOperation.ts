...
export const deviceOperationApi = {
  ...
  confirm: async (id: number, actionId: string, params: Record<string, unknown>, reason: string) => unwrap<ConfirmationGrant>(await client.post<unknown, ApiEnvelope<ConfirmationGrant>>(`/api/v1/edge-devices/${id}/actions/${actionId}/confirm`, { params, reason })),
  resolve: async (commandId: string, outcome: ManualResolutionOutcome, reason: string) => unwrap<DeviceOperation>(await client.post<unknown, ApiEnvelope<DeviceOperation>>(`/api/v1/device-operations/${commandId}/resolve`, { outcome, reason })),
}
